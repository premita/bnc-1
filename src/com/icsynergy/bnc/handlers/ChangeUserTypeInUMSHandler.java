...
public class ChangeUserTypeInUMSHandler implements PostProcessHandler {
		 ...
		 if (orchestration != null) {
	        ...
	        try {	    	   
				...
				if(!isNullOrEmpty(userType) && !isNullOrEmpty(olduserType)){
					if(olduserType.equals(Constants.UserAttributes.HR_PENDING) && userType.equals(Constants.UserAttributes.EMP)){
			            log.fine("user Type changed from "+olduserType+" to "+userType+", setting User type in IAMUMS");		
						setUserTypeInUMS(empNo, oimConnection);
					}
					else{
			            log.fine("user Type changed from "+olduserType+" to "+userType+", no need to set User type in IAMUMS");		
					}
				}
	...
	public Connection getDatabaseConnection(String dbJNDIName) {
	   ...
	   private Object getNewUserStates(Orchestration orchestration) {
	       ...
	       if (interEventData != null)
	           newUserStates =
	                   interEventData.get(Constants.UserAttributes.NEW_USER_STATE);
	   ...
	   private Object getOldUserStates(Orchestration orchestration) {
	       ...
	       try {
	           ...
	           if (interEventData != null)
	               oldUserStates =
	                       interEventData.get(Constants.UserAttributes.CURRENT_USER);
	       } catch (Exception e) {
        	   log.log(Level.SEVERE, "Exception working with contexts", e);
	       }
			...
			if (bulkOrchestration != null) {
	    	    ...
	    	    oimConnection = getDatabaseConnection(umsdbJNDIName);
		           for (int i = 0; i < bulkParameters.length; i++) {
		        	        HashMap<String, Serializable> parameters =  bulkParameters[i];
							...
							log.fine( "newUserState In Bulk operation " + newUserState +" oldUserState in Bulk operation "+oldUserState);					     
							...
							if(!isNullOrEmpty(userType) && !isNullOrEmpty(olduserType)){
								if(olduserType.equalsIgnoreCase(Constants.UserAttributes.HR_PENDING) && userType.equalsIgnoreCase(Constants.UserAttributes.EMP)){
						            log.fine("user Type changed from 'HR Pending' to 'Employee', setting User type in IAMUMS");		
									setUserTypeInUMS(empNo, oimConnection);
								}else{
					             log.fine("user Type changed from "+olduserType+" to "+userType+", no need to set User type in IAMUMS");		
								}
